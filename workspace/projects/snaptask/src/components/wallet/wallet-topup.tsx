...
import { eurosToCents, formatEurFromCents } from "@/lib/utils";
import type { Profile } from "@/types/database";

/** Preset top-ups in euros (converted to cents on submit). */
const PRESETS_EUR = [1, 5, 10, 25];
...
export function WalletTopUp({
  ...
  function submit() {
    ...
    startTransition(async () => {
      ...
      setMessage(`Added ${formatEurFromCents(amountCents)} to your wallet.`);
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {PRESETS_EUR.map((preset) => (
            ...
            >
              {formatEurFromCents(eurosToCents(preset))}
            </button>
          ))}
        </div>
        <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <Button onClick={submit} disabled={pending || amountEur < 1}>
